...
class MultiS3ShardRetriever:
    ...
    def __init__(self, zones: Iterable[S3ZoneConfig], n_bits: int = 8) -> None:
        self._zones: List[S3ZoneConfig] = list(zones)
        if not self._zones:
            raise ValueError("At least one S3 zone must be configured")
        self._n_bits = n_bits
        self._validate_zones()
        self._retrievers: List[S3ShardRetriever] = [S3ShardRetriever(z.s3_config, n_bits=n_bits) for z in self._zones]
