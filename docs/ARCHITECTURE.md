...
- **DATA**: concatenated file payloads (may be compressed).
- **INDEX**: entry_count followed by per-file records:
  - UID name length + bytes
  - offset + length (compressed)
  - codec byte
...
- S3 retriever:
  - range-GET footer (fixed-size) → parse index offset/size
  - range-GET index → parse entries
  - range-GET payload slice → decompress per entry
- In-memory index cache avoids repeated footer/index fetches; payload still uses range GET.
